...
const path = {
  //создаем переменную path (путь)
  src: {
    html: ["app/*.html"],

    styles: ["app/" + syntax + "/**/*." + syntax + ""],

    js: [
      //!!! в той последовательности в которой подключали их!!!
...
gulp.task("js", function() {
  return (
    gulp
      .src(path.src.js) // 1.Берем все необходимые файлы и библиотеки
  ...
  ); //3. Перезагрузка тех файлов за которыми мы следим
});
...
gulp.task("styles", function() {
  return (
    gulp
      .src(path.src.styles) // 1.Берем все необходимые файлы и библиотеки
...
gulp.task("html", function() {
  return gulp
    .src(path.src.html) // 1.Берем файл index.html
...
gulp.task("img", function() {
  return gulp
    ...
    .pipe(browserSync.reload({ stream: true })); // перезагрузка с добавленными картинками
});

/* Создаем таск clean который, удаляет папку dist*/
gulp.task("clean", function() {
  return gulp
    .src("dist") //1. берем папку dist
...
if (gulpversion == 4) {
  ...
  gulp.task(
    "default",
    gulp.series("dist", gulp.parallel("browser-sync", "watch"))
  );
}
